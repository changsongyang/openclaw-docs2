...
title: 文件地圖
x-i18n:
    generated_at: "2026-07-26T07:50:23Z"
    model: gpt-5.6
...
此檔案由 `docs/**/*.md` 和 `docs/**/*.mdx` 的標題產生，以協助代理程式瀏覽文件樹狀結構。
...
- 路由：/channels/discord-activities
- 標題：
  - H2：必要條件
  - H2：設定
  - H2：安全性模型
  - H2：疑難排解
  - H3：活動顯示「閘道離線」
  - H3：Discord 開啟空白頁面或回報 blocked:csp
  - H3：「小工具無法使用」
  - H3：「你無法在此頻道啟動活動」
...
- 標題：
  ...
  - H3：由機器人撰寫的訊息
  - H2：取得群組／使用者 ID
  - H3：群組 ID（`chat_id`，格式：`oc_xxx`）
  - H3：使用者 ID（`open_id`，格式：`ou_xxx`）
  - H2：常用命令
  - H2：疑難排解
  - H3：機器人在群組聊天中沒有回應
  - H3：機器人未收到訊息
...
- 標題：
  ...
  - H2：私有 API 動作
  - H2：寫入設定
  - H2：合併分次傳送的私訊（在同一編寫內容中包含命令 + URL）
  - H2：橋接器或閘道重新啟動後的傳入訊息復原
  - H3：操作人員可見的訊號
  - H3：遷移
  - H2：疑難排解
  - H2：設定參考指引
...
- 標題：
  ...
  - H2：快速設定
  - H2：這是什麼
  - H2：設定路徑 A：連結現有 Signal 帳號（QR Code）
...
- 標題：
  ...
  - H2：注意事項
  - H2：權杖偏移復原檢查清單
  - H2：Paperclip／`openclaw_gateway` 首次執行核准
  - H2：相關內容
...
- 標題：
  ...
  - H3：其他隨附的提供者外掛
  - H4：值得瞭解的特殊行為
  - H2：透過 models.providers 使用提供者（自訂／基礎 URL）
  - H3：Moonshot AI（Kimi）
  - H3：Kimi Coding
...
- 標題：
  ...
  - H2：Claude 使用 1Password 進行瀏覽器登入
  - H2：安全性注意事項
  - H2：疑難排解
...
- 標題：
  ...
  - H2：運作方式
  - H2：逾時與長時間執行的工作
  - H3：Claude 命令列介面的特定事項
  - H3：Claude 瀏覽器工具與 1Password 登入
  - H2：工作階段
...
- 標題：
  ...
  - H2：常見模式
  - H3：共用 Skill 基準設定搭配單一覆寫
  - H3：多平台設定
  - H3：自動核准受信任的節點網路
  - H3：安全的私訊模式（共用收件匣／多使用者私訊）
...
- 標題：
  ...
  - H2：項目（輸入）
  - H3：訊息
  - H3：`function_call_output`（回合式工具）
  - H3：推理與 `item_reference`
  - H2：工具（用戶端函式工具）
  - H2：圖片（`input_image`）
  - H2：檔案（`input_file`）
  - H2：檔案與圖片限制
...
- 標題：
  ...
  - H3：子代理程式委派防護措施
  - H3：唯讀模式
  - H2：各代理程式的存取設定檔（多代理程式）
  - H3：完整存取權（無沙箱）
  - H3：唯讀工具與唯讀工作區
  - H3：無檔案系統／Shell 存取權（允許供應商訊息傳遞）
  - H2：瀏覽器控制風險
...
- 路由：/gateway/security/rate-limiting
- 標題：
  - H2：驗證嘗試（驗證前）
  - H3：瀏覽器來源連線
  - H3：網路鉤子
...
- 標題：
  ...
  - H2：分裂安裝與新版設定防護
  - H2：回復版本後的通訊協定不相符
  - H2：Skills 符號連結因路徑逸出而遭略過
  - H2：Anthropic 429：長內容需要額外用量
  - H2：上游 403 封鎖回應
  - H2：本機 OpenAI 相容後端通過直接探測，但代理程式執行失敗
  - H2：沒有回覆
  - H2：儀表板控制介面連線
  - H3：驗證詳細代碼速查表
  - H2：閘道服務未執行
  - H2：macOS 閘道無聲停止回應，碰觸儀表板後才恢復
  - H2：macOS launchd 監督程式因重複的閘道／節點 LaunchAgent 而陷入迴圈
  - H2：閘道在高記憶體用量期間結束
  - H2：閘道拒絕無效設定
  - H2：閘道探測警告
  - H2：頻道已連線，但訊息未流通
...
- 標題：
  ...
  - H3：端對端（儲存庫彙總）
  - H3：端對端（閘道冒煙測試）
  - H3：端對端（Control UI 模擬瀏覽器）
  - H3：端對端：OpenShell 後端冒煙測試
  - H3：即時（實際供應商 + 實際模型）
  - H2：我該執行哪個套件？
  - H2：即時（會存取網路）測試
...
- 標題：
  ...
  - H2：自動偵測（預設）
  - H2：設定範例
  - H3：供應商與命令列介面備援（OpenAI + Whisper 命令列介面）
  - H3：僅使用供應商（Deepgram）
...
- 標題：
  - H2：需求
  - H2：電腦代理程式工具
  - H2：Windows 與 Linux（實驗性，透過 cua-driver）
  - H3：疑難排解
...
- 路由：/plan/swarms
- 標題：
  - H1：群集——程式碼模式中的代理程式分流與協調
  - H2：1. 內容與原因
  - H2：2. 決策（維護者，2026-07-17）
  - H2：3. 架構概觀
  - H2：4. 設定閘門（v1）
  - H2：5. 核心：收集器模式的衍生與 `agents_wait`（v1）
  - H3：5.1 `sessions_spawn` 新增項目（全部以群集已啟用為前提）
  - H3：5.2 核准採取失敗關閉
  - H3：5.3 `agents_wait` 工具（新增，受閘門控管）
  - H3：5.4 上限強制執行
  - H2：6. 測試合約（v1，路線 A）
  - H2：7. QuickJS 客體介面（路線 B，核心完成後）
  - H2：8. Codex 測試框架投影（後續路線）
  - H2：9. 持久化與保留
  - H2：10. 進度介面（“圓點”）——後續路線
  - H2：11. 實驗室頁面（控制使用者介面，獨立路線）
  - H2：12. 配置（後續）
  - H2：13. 非目標
  - H2：14. 建置階段／PR 切分
...
- 路由：/plugins/reference/cua-computer
- 標題：
  - H1：Cua Computer 外掛
  - H2：發行方式
  - H2：功能介面
...
- 路由：/plugins/reference/linux-canvas
- 標題：
  - H1：Linux Canvas 外掛
  - H2：發行方式
  - H2：功能介面
...
- 路由：/plugins/reference/teams-meetings
- 標題：
  - H1：Microsoft Teams 會議外掛
  - H2：發佈
  - H2：介面
  - H2：相關文件
...
- 路由：/plugins/reference/zoom-meetings
- 標題：
  - H1：Zoom 會議外掛
  - H2：發佈
  - H2：介面
...
- 標題：
  ...
  - H2：思考預設值（Claude Opus 5、Sonnet 5、Mythos 5、Fable 5、4.8 與 4.6）
...
- 標題：
  ...
  - H3：階段 0：凍結邊界
  - H3：階段 1：完成全域控制平面
  - H3：階段 2：導入各代理程式專用資料庫
  - H3：階段 3：取代工作階段儲存區 API
  - H3：階段 4：遷移逐字稿、ACP 串流、軌跡與 VFS
  - H3：階段 5：備份、還原、壓縮與驗證
  - H3：階段 6：工作程序執行階段
  - H3：階段 7：刪除舊架構
  - H2：備份與還原
...
- 標題：
  ...
  - H2：完整性檢查
  - H2：疑難排解
  - H3：為何更新至 2026.7.2 後無法降回舊版
  - H3：閘道因結構描述版本較新的錯誤而拒絕啟動
  - H3：完整性驗證失敗後，資料庫遭到隔離
  - H2：不支援降級
  - H3：範例：代理程式結構描述從 11 降至 9
...
- 路由：/reference/pull-request-review-flow
- 標題：
  - H2：Barnacle
  - H2：ClawSweeper
  - H2：在審查期間改善 PR
...
- 標題：
  ...
  - H4：T-EXEC-001：直接提示詞注入
  - H4：T-EXEC-002：間接提示詞注入
...
- 路由：/start/onboarding-redesign
- 標題：
  - H1：新手引導重新設計實作計畫
  - H2：核心願景
  - H2：目前已發布的流程（第 1～3 階段完成後）
  - H2：階段
  - H2：各階段實作備註
  - H3：第 1 階段 — 應用程式建議（PR #109668）
  - H3：第 2 階段 — 命令列介面管理員主幹（PR #109841）
  - H3：第 3 階段 — 瀏覽器優先交接（PR #110054，已合併）
  - H3：第 4 階段 — 網頁管理員介面（已合併：#110141、#110242）
  - H3：第 5 階段 — 孵化與啟動（已合併：#110173、#110331）
  - H3：第 6 階段 — 管理員在場狀態（PR1 已合併：#110269；評論／召喚屬於 PR2）
  - H3：第 7 階段 — 韌性（建置前需要負責人決策）
  - H2：測試與落地操作手冊（得來不易；進行第 4～6 階段前請先閱讀）
  - H2：決策紀錄
  - H2：已知缺口與後續工作
...
- 標題：
  ...
  - H2：客製化策略（避免更新造成影響）
  - H2：從此存放庫執行閘道
  - H2：穩定版工作流程（以 macOS 應用程式為優先）
  - H2：尖端版工作流程（在終端機中執行閘道）
  - H3：0）（選用）也從原始碼執行 macOS 應用程式
  - H3：1）啟動開發用閘道
  - H3：2）讓 macOS 應用程式連線至正在執行的閘道
  - H3：3）驗證
  - H3：常見陷阱
  - H2：認證資訊儲存位置圖
  - H2：更新（且不破壞你的設定）
...
- 標題：
  ...
  - H3：第 2 層：驗證 WSL2 可以連線至該 Windows 端點
...
- 標題：
  ...
  - H3：將核准傳送至工作階段時，該工作階段中的任何人都可以核准嗎？
  - H2：相關內容
...
- 標題：
  ...
  - H3：衍生行為
  - H2：上下文模式
  - H2：工具：`sessions_spawn`
  - H3：委派提示詞模式
  - H3：工具參數
  - H3：任務名稱與目標指定
  - H2：工具：`sessions_yield`
  - H2：工具：subagents
  - H2：綁定討論串的工作階段
  - H3：支援討論串的頻道
  - H3：快速流程
...
- 標題：
  ...
  - H2：互動層級
  - H2：小工具模型與託管
  - H3：小工具託管內容；MCP 應用程式是其中一種內容類型
  - H3：外掛功能宣告
  - H3：建模後的剩餘項目：WebRTC 資料頻道
  - H3：逐字記錄顯示：單張小工具卡片
  - H3：伺服器來源的小工具（已釘選的 MCP 應用程式）
  - H3：WorkBoard 整合
  - H2：版面配置：流動式網格
  - H2：資料模型（各代理程式資料庫）
  - H2：通訊協定介面
