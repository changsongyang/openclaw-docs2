...
title: 文档地图
x-i18n:
    generated_at: "2026-07-26T06:13:00Z"
    model: gpt-5.6
...
- 标题：
  ...
  - H2：安全模型
  - H2：故障排查
  - H3：Activity 显示“Gateway 网关离线”
  - H3：Discord 打开空白页面或报告 blocked:csp
  - H3：“小组件不可用”
  - H3：“你无法在此频道中启动 Activities”
...
- 标题：
  ...
  - H3：机器人发送的消息
  - H2：获取群组/用户 ID
  - H3：群组 ID（`chat_id`，格式：`oc_xxx`）
  - H3：用户 ID（`open_id`，格式：`ou_xxx`）
  - H2：常用命令
  - H2：故障排查
  - H3：机器人在群聊中不响应
  - H3：机器人收不到消息
  - H3：二维码设置在 Feishu 移动应用中没有响应
  - H3：App Secret 泄露
  - H2：高级配置
  - H3：多个账户
  - H3：消息限制
...
- 标题：
  ...
  - H2：说明
  - H2：令牌漂移恢复检查清单
  - H2：Paperclip / `openclaw_gateway` 首次运行审批
  - H2：相关内容
...
- 标题：
  ...
  - H3：其他内置提供商插件
  - H4：值得了解的特殊之处
  - H2：通过 models.providers 配置提供商（自定义/基础 URL）
  - H3：Moonshot AI（Kimi）
  - H3：Kimi Coding
  - H3：火山引擎（豆包）
  - H3：BytePlus（国际版）
...
- 标题：
  - H2：要求
  - H2：使用 op 解析配置密钥
  - H2：无头 Gateway 网关的服务账户设置
  - H2：供智能体使用的 1password Skill
  - H2：使用 1Password 为 Claude 进行浏览器登录
  - H2：安全说明
  - H2：故障排查
...
- 标题：
  ...
  - H2：项目（输入）
  - H3：消息
  - H3：`function_call_output`（基于轮次的工具）
  - H3：推理和 `item_reference`
  - H2：工具（客户端函数工具）
  - H2：图像（`input_image`）
  - H2：文件（`input_file`）
  - H2：文件和图像限制
...
- 标题：
  ...
  - H2：长上下文需要额外用量时出现 Anthropic 429
  - H2：上游 403 阻止响应
  ...
  - H3：身份验证详细代码速查表
  - H2：Gateway 网关服务未运行
  - H2：macOS Gateway 网关悄然停止响应，操作仪表板后恢复
  - H2：存在重复 Gateway 网关/节点 LaunchAgent 时的 macOS launchd 监管循环
  - H2：Gateway 网关在内存使用量较高时退出
  - H2：Gateway 网关拒绝了无效配置
  - H2：Gateway 网关探测警告
  - H2：渠道已连接，但消息未流转
  - H2：定时任务和 Heartbeat 递送
  - H2：节点已配对，但工具失败
  - H2：浏览器工具失败
  - H2：如果升级后某些功能突然失效
  - H2：相关内容
...
- 标题：
  - H2：实时测试与你的真实 Gateway 网关
  - H2：实时测试：本地冒烟测试命令
  - H2：实时测试：Android 节点能力扫描
...
- 标题：
  - H2：最初的六十秒
  - H2：助手能力似乎受限或缺少工具
  - H2：Anthropic 长上下文 429 错误
  - H2：本地 OpenAI 兼容后端可直接运行，但在 OpenClaw 中失败
  - H2：安装插件时因缺少 OpenClaw 扩展而失败
  - H2：安装策略阻止安装或更新插件
  - H2：插件存在，但因所有权可疑而被阻止
  - H2：决策树
  - H2：相关内容
...
- 标题：
  - H2：推荐的默认方案（适合大多数用户）
  - H2：macOS 虚拟机选项
  - H3：Apple Silicon Mac 上的本地虚拟机（Lume）
  - H3：托管式 Mac 提供商（云端）
...
- 标题：
  - H2：要求
  - H2：计算机智能体工具
  - H2：Windows 和 Linux（实验性，通过 cua-driver）
  - H3：故障排查
...
- 标题：
  ...
  - H3：启动节点主机（前台）
  - H3：通过 SSH 隧道连接远程 Gateway 网关（环回绑定）
  - H3：启动节点主机（服务）
  - H3：配对和命名
  - H3：由节点托管的 MCP 服务器
  - H3：由节点托管的 Skills
...
- 标题：
  ...
  - H2：组件
  - H3：1. 环境状态机和提供商契约
  - H3：2. 工作节点引导：在主机上安装 OpenClaw
  - H3：3. 传输：所有操作均通过 SSH
  - H3：4. 工作节点协议（专用协议；不是节点协议）
  - H3：5. 会话后端 RPC
  - H3：6. 工作区同步
  - H3：7. 放置状态机、会话和 UI
  - H2：分派和交接
...
- 路由：/plan/swarms
- 标题：
  - H1：Swarm——代码模式中的智能体扇出和编排
  - H2：1. 内容和原因
  - H2：2. 决策（维护者，2026-07-17）
  - H2：3. 架构概览
  - H2：4. 配置门控（v1）
  - H2：5. 核心：收集器模式生成 + `agents_wait`（v1）
  - H3：5.1 `sessions_spawn` 新增项（全部受 Swarm 启用状态门控）
  - H3：5.2 审批采用故障关闭
  - H3：5.3 `agents_wait` 工具（新增，受门控）
  - H3：5.4 上限执行
  - H2：6. 测试契约（v1，通道 A）
  - H2：7. QuickJS 客体接口（通道 B，在核心之后）
  - H2：8. Codex harness 投影（后续通道）
  - H2：9. 持久化和保留
  - H2：10. 进度界面（“圆点”）——后续通道
  - H2：11. 实验室页面（Control UI，独立通道）
  - H2：12. 放置（后续）
  - H2：13. 非目标
  - H2：14. 构建阶段/PR 拆分
...
- 标题：
  ...
  - H2：入门指南
  - H2：跨计算机使用 Claude 会话
  - H2：思考默认设置（Claude Opus 5、Sonnet 5、Mythos 5、Fable 5、4.8 和 4.6）
...
- 标题：
  ...
  - H2：完整性检查
  - H2：故障排除
  - H3：为何更新到 2026.7.2 后无法回退
  - H3：Gateway 网关因架构版本较新错误而拒绝启动
  - H3：数据库在完整性验证失败后被隔离
  - H2：不支持降级
  - H3：示例：Agent 架构从 11 降至 9
...
- 标题：
  ...
  - H2：亮点
  - H3：Control UI 全面革新：聊天、会话、工作区和用量
  - H3：从安装到首次聊天，设置更轻松
  - H3：官方应用
  - H4：应用通用改进
...
- 标题：
  - H1：新手引导重新设计实施计划
  - H2：终极目标
  - H2：当前已发布流程（第 1-3 阶段完成后）
  - H2：阶段
  - H2：各阶段实施说明
  - H3：第 1 阶段 — 应用推荐（PR #109668）
  - H3：第 2 阶段 — CLI 管理主干（PR #109841）
  - H3：第 3 阶段 — 浏览器优先交接（PR #110054，已合并）
  - H3：第 4 阶段 — Web 管理界面（已合并：#110141、#110242）
  - H3：第 5 阶段 — 孵化与引导启动（已合并：#110173、#110331）
  - H3：第 6 阶段 — 管理者在线状态（PR1 已合并：#110269；评论/召唤属于 PR2）
  - H3：第 7 阶段 — 韧性（构建前需要所有者作出决定）
  - H2：测试与落地操作手册（经验来之不易；开始第 4-6 阶段前请阅读）
  - H2：决策日志
  - H2：已知缺口与后续工作
...
- 标题：
  ...
  - H3：2）将 macOS 应用连接到正在运行的 Gateway 网关
  - H3：3）验证
  - H3：常见陷阱
  - H2：凭据存储映射
  - H2：更新（不破坏现有设置）
...
- 标题：
  ...
  - H2：交互层级
  - H2：小组件模型和托管
  - H3：小组件托管内容；MCP 应用是其中一种内容类型
  - H3：插件能力声明
  - H3：建模后的剩余项：WebRTC 数据渠道
  - H3：记录显示：单个小组件卡片
  - H3：服务器提供的小组件（固定的 MCP 应用）
  - H3：WorkBoard 集成
  - H2：布局：流式网格
  - H2：数据模型（每个智能体的数据库）
  - H2：协议接口
